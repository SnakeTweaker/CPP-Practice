...
// Define URL Path annotation for gameusers
@Path("/gameusers/")
...
public class GameUserRESTController {
    ...
    // User authentication. Allow API access to users with USER role
    @RolesAllowed({ "USER" })
    @GET
    @Path("/{id}")
    ...
    // Admin authentication. Allow API access to admin with ADMIN role
    @RolesAllowed({ "ADMIN" })
    @POST
